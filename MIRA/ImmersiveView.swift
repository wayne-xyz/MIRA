    ...
    var body: some View {
        RealityView { content in
            if let angelEntity = try? await Entity(named: "Angel.usdz", in: realityKitContentBundle) {
                ...
                angelEntity.position = SIMD3<Float>(x: 0.0, y: -0.5, z: -1.0) // Adjust position as needed
            ...
            if let devilEntity = try? await Entity(named: "Devil.usdz", in: realityKitContentBundle) {
                ...
                // Scale down the model
                devilEntity.scale = SIMD3<Float>(0.1, 0.1, 0.1) // Adjust scale as needed
                
                // Move the model to a new position
                devilEntity.position = SIMD3<Float>(x: 0.0, y: -0.5, z: -125.0) // Adjust position as needed
